    ...
    for (const user of userdata) {
        if (user.username === username) {
            if (user.password === password) {
                ...
                // 等待 token 生成
                const token = await tokenUtil.generateToken({ guid: user.guid }, 60 * 60 * 24);
...
async function loginWithToken(token) {
	let code = await tokenUtil.validateToken(token);
	let data = {
		// 不存在则报错
		code: 702,
		msg: "用户token出错",
...
async function changePwd(token, uname, old_pwd, new_pwd) {
    // 先验证token
    let code = await tokenUtil.validateToken(token);
    let data = {
        // 不存在则报错
