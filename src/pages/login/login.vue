<template>
  <view class="main">
    <view class="header">
      ...
      <view class="nav">
        <view
          :class="['nav_item', navidx === 0 ? 'nav_left nav_now' : 'nav_right']"
          @click="navidx = 0"
        >
          登录
        </view>
        <view
          :class="['nav_item', navidx === 1 ? 'nav_right nav_now' : 'nav_left']"
          @click="navidx = 1"
        >
          注册
        </view>
    ...
    <view :class="['body', navidx === 0 ? 'bg-left' : 'bg-right']">
      <view class="form_box login" v-if="navidx === 0">
        <u--form :model="formRef">
          <u-form-item prop="userInfo.uname">
            <view
              :class="[
                'label',
                formRef.userInfo.uname != '' || isFocus0 ? 'focused' : 'blured',
              ]"
              >用户名</view
            >
            <u-input 
          ...
          <u-form-item prop="userInfo.password">
            <view
              :class="[
                'label',
                formRef.userInfo.password != '' || isFocus1 ? 'focused' : 'blured',
              ]"
              >密码</view
            >
            <u-input
        ...
        <view class="visitor-entrance" @click="visitLogin"> >游客登入< </view>
      </view>
...
let isFocus1 = ref(false); // 密码输入框焦点状态
// 定义表单数据
...
function submit(): void {
  ...
  if (checkLogin()) {
    ...
    let req = {
      username: formRef.userInfo.uname,
      password: formRef.userInfo.password,
    };
    $axios
      .get("/login", { params: req })
      .then((res) => {
        ...
        if (res.status === 200) {
          ...
          if (res.data.code === 200) {
            userStore.login(res.data.udata);
            setTimeout(() => {
              uni.switchTab({
...
function checkLogin(): boolean {
  if (test.isEmpty(formRef.userInfo.uname)) {
    toast("用户名不能为空");
    return false;
  }
  if (test.isEmpty(formRef.userInfo.password)) {
    toast("密码不能为空");
    return false;
  }
  return true;
...
@font-face {
  font-family: "ZoomlaTouyinghei-A032"; /*逐浪投影透视黑体*/
  src: url("https://code.z01.com/font/ZoomlaTouyinghei-A032.eot?#iefix"); /* IE9 */
  src: url("https://code.z01.com/font/ZoomlaTouyinghei-A032.eot?#iefix")
      format("embedded-opentype"),
    /* IE6-IE8 */ url("https://code.z01.com/font/ZoomlaTouyinghei-A032.woff")
      format("woff"),
    /* chrome、firefox */
      url("https://code.z01.com/font/ZoomlaTouyinghei-A032.ttf")
      format("truetype"),
    /* chrome、firefox、opera、Safari, Android, iOS 4.2+ */
      url("https://code.z01.com/font/ZoomlaTouyinghei-A032.svg#ZoomlaTouyinghei-A032")
      format("svg"); /* iOS 4.1- */
  font-style: normal;
  font-weight: normal;
}
...
.title {
  ...
  letter-spacing: 8px;
  margin-left: 32px;
  text-shadow: 0 5px 1px #006b6f, 0 10px 5px #006b6f, 0 -2px 0px $theme-color,
